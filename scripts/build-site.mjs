...
function renderEditorialSections(sections) {
  ...
  const blocks = sections.map(block => {
    ...
    if (type === "split") {
      ...
      const side = block.imageSide === "right" ? " image-right" : " image-left";
      const fit = block.fit === "cover" ? " fit-cover" : " fit-contain";
      const heading = block.heading ? `<h2 class="editorial-section-title">${escapeHtml(block.heading)}</h2>` : "";
      return `<section class="editorial-section editorial-flow${side}${fit}"><figure class="editorial-flow-media"><img src="${escapeAttr(block.image)}" alt="${escapeAttr(block.alt || block.heading || "Blog görseli")}" loading="lazy" decoding="async" onerror="this.closest('figure')?.remove();this.closest('.editorial-flow')?.classList.add('media-missing')"></figure>${heading}${renderRichText(block.body || "")}</section>`;
    }
    if (type === "gallery") {
...
function blogPage(post) {
  ...
  return `<!doctype html><html lang="tr"><head>
    ...
    </main>${commonFooter()}<script src="/assets/js/elci-system.js" defer></script><script src="/assets/js/elci-blog-v57.js?v=20260723-editorial-1" defer></script><script src="/assets/js/elci-blog-v58.js?v=20260724-reading-flow-1" defer></script>
