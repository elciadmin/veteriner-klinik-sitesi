(() => {
  ...
  function renderEditorialBlock(block,index,total) {
    ...
    if(type==='split') fields=`${inputField(`block.${index}.heading`,'Bölüm başlığı (isteğe bağlı)',block.heading||'','text','',true)}${textField(`block.${index}.body`,'Metin (isteğe bağlı)',block.body||'','Metin veya görselden biri yeterlidir. İkisini doldurursanız yan yana görünür.',true,6)}${blockImageField(`block-image-${index}`,block.image||'','Görsel (isteğe bağlı)')}${inputField(`block.${index}.alt`,'Görsel açıklaması',block.alt||'','text','',true)}${selectField(`block.${index}.imageSide`,'Görsel konumu',block.imageSide||'right',[{label:'Sağda',value:'right'},{label:'Solda',value:'left'}])}${selectField(`block.${index}.fit`,'Görsel görünümü',block.fit||'cover',[{label:'Fotoğraf gibi kırp',value:'cover'},{label:'Tamamını göster',value:'contain'}])}`;
  ...
  function syncBlocksToState() {
    ...
    state.edit.data.editorialSections=nodes.map(block=>{
      ...
      if(type==='split')return{type,heading:get('heading'),body:get('body'),image:block.querySelector(`[name="block-image-${index}"]`)?.value||'',alt:get('alt'),imageSide:get('imageSide')||'right',fit:get('fit')||'cover'};
  ...
  function addEditorialBlock(type) {
    const current=collectCurrentForm();
    const defaults={text:{type:'text',heading:'',body:''},image:{type:'image',image:'',alt:'',caption:'',size:'wide',fit:'contain'},split:{type:'split',heading:'',body:'',image:'',alt:'',imageSide:'right',fit:'cover'},gallery:{type:'gallery',layout:'auto',fit:'cover',images:[]},callout:{type:'callout',tone:'info',heading:'',body:''},steps:{type:'steps',heading:'',items:[]}};
