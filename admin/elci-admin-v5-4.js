(() => {
  ...
  function contentModeSelector(mode) {
    return `<section class="form-card content-mode-card" id="contentModeCard"><div class="section-heading"><span class="section-icon"><i class="fa-solid fa-wand-magic-sparkles"></i></span><div><h2>Yazı düzeni</h2><p>Normal blog yazılarında otomatik dergi düzenini seçili bırakın. Site görseli ve metni kendisi dengeler.</p></div></div><div class="content-mode-grid"><label class="content-mode-option ${mode==='standard'?'selected':''}"><input type="radio" name="contentMode" value="standard" ${mode==='standard'?'checked':''}><span class="mode-icon"><i class="fa-solid fa-wand-magic-sparkles"></i></span><span><strong>Otomatik dergi düzeni</strong><small>Başlık, özet, kapak ve tek metin alanı. Görsel yazıyla bütünleşir; devamı tam genişlikte akar.</small></span><i class="fa-solid fa-circle-check mode-check"></i></label><label class="content-mode-option ${mode==='visual'?'selected':''}"><input type="radio" name="contentMode" value="visual" ${mode==='visual'?'checked':''}><span class="mode-icon"><i class="fa-regular fa-images"></i></span><span><strong>Gelişmiş blok düzeni</strong><small>Birden fazla görsel, galeri, bilgi kutusu veya adım listesi gereken özel yazılar için.</small></span><i class="fa-solid fa-circle-check mode-check"></i></label></div></section>`;
  }
  function standardContentSection(data) {
    return `<section class="form-card"><div class="section-heading"><span class="section-icon"><i class="fa-solid fa-pen-nib"></i></span><div><h2>Yazının metni</h2><p>Siz yalnızca metni yazın veya yapıştırın. Yazı tipi, görsel ölçüsü, boşluklar ve telefon görünümü otomatik ayarlanır.</p></div></div><div class="field full"><label for="f-content">Metin</label><div class="rich-toolbar"><button type="button" data-format="h2">Büyük başlık</button><button type="button" data-format="h3">Alt başlık</button><button type="button" data-format="bold">Kalın</button><button type="button" data-format="list">Madde</button><button type="button" data-format="number">Numara</button><button type="button" data-format="quote">Bilgi notu</button></div><textarea class="rich-text" id="f-content" name="content" placeholder="Yazınızı buraya yazın veya hazır metni yapıştırın…">${esc(data.content||'')}</textarea><small>Paragraflar arasına bir boş satır bırakın. Bölüm başlığı için “Büyük başlık” düğmesini kullanabilirsiniz.</small></div></section>`;
  }
  ...
  function validateEditor(name,data,action) {
    ...
    if(name==='blog'&&!isDraft){
      const mode=blogContentMode(data);
      if(!mode)return'Yazı düzenini seçin.';
      if(mode==='standard'&&!String(data.content||'').trim())return'Yazının metnini ekleyin.';
      if(mode==='visual'){
