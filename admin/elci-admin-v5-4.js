(() => {
  ...
  function renderEditorialBlock(block,index,total) {
    ...
    const typeLabel={text:'Sadece metin',image:'Tek görsel',split:'Görsel + metin',gallery:'Görsel galerisi',callout:'Bilgi kutusu',steps:'Numaralı adımlar'}[type]||'Yazı bölümü';
    let fields='';
    ...
    if(type==='split') fields=`${inputField(`block.${index}.heading`,'Bölüm başlığı (isteğe bağlı)',block.heading||'','text','',true)}${textField(`block.${index}.body`,'Metin (isteğe bağlı)',block.body||'','Metin veya görselden biri yeterlidir. İkisini doldurursanız yan yana görünür.',true,6)}${blockImageField(`block-image-${index}`,block.image||'','Görsel (isteğe bağlı)')}${inputField(`block.${index}.alt`,'Görsel açıklaması',block.alt||'','text','',true)}${selectField(`block.${index}.imageSide`,'Görsel konumu',block.imageSide||'left',[{label:'Solda — dergi düzeni (önerilen)',value:'left'},{label:'Sağda',value:'right'}])}${selectField(`block.${index}.fit`,'Görsel görünümü',block.fit||'contain',[{label:'Tamamını göster — afiş / belge / infografik (önerilen)',value:'contain'},{label:'Fotoğraf gibi kırp',value:'cover'}])}`;
  ...
  function syncBlocksToState() {
    ...
    state.edit.data.editorialSections=nodes.map(block=>{
      ...
      if(type==='split')return{type,heading:get('heading'),body:get('body'),image:block.querySelector(`[name="block-image-${index}"]`)?.value||'',alt:get('alt'),imageSide:get('imageSide')||'left',fit:get('fit')||'contain'};
  ...
  function addEditorialBlock(type) {
    const current=collectCurrentForm();
    const defaults={text:{type:'text',heading:'',body:''},image:{type:'image',image:'',alt:'',caption:'',size:'wide',fit:'contain'},split:{type:'split',heading:'',body:'',image:'',alt:'',imageSide:'left',fit:'contain'},gallery:{type:'gallery',layout:'auto',fit:'cover',images:[]},callout:{type:'callout',tone:'info',heading:'',body:''},steps:{type:'steps',heading:'',items:[]}};
