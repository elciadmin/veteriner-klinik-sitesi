(() => {
  ...
  function featureCardField(index,slot,item={}) {
    ...
    return `<div class="feature-card-editor"><strong>${slot+1}. kutucuk</strong><div class="form-grid">${selectField(`block.${index}.card.${slot}.icon`,'Simge',item.icon||'paw',icons)}${inputField(`block.${index}.card.${slot}.title`,'Başlık',item.title||'','text','',true)}${textField(`block.${index}.card.${slot}.body`,'Kısa metin',item.body||'','Kutucuklar isteğe bağlıdır; yalnızca dolu olanlar yayımlanır.',true,4)}${blockImageField(key,item.image||'','Görsel (isteğe bağlı)')}${inputField(`block.${index}.card.${slot}.alt`,'Görsel açıklaması',item.alt||'','text','',true)}</div></div>`;
  }
  ...
  function syncBlocksToState() {
    ...
    state.edit.data.editorialSections=nodes.map(block=>{
      ...
      if(type==='cards')return{type,items:[0,1,2].map(slot=>({icon:get(`card.${slot}.icon`)||'paw',title:get(`card.${slot}.title`),body:get(`card.${slot}.body`),image:block.querySelector(`[name="block-card-${index}-${slot}"]`)?.value||'',alt:get(`card.${slot}.alt`)})).filter(item=>item.title||item.body||item.image)};
  ...
  function addEditorialBlock(type) {
    const current=collectCurrentForm();
    const defaults={text:{type:'text',heading:'',body:''},image:{type:'image',image:'',alt:'',caption:'',size:'wide',fit:'contain'},split:{type:'split',heading:'',body:'',image:'',alt:'',caption:'',imageSide:'right',fit:'cover'},gallery:{type:'gallery',layout:'auto',fit:'cover',images:[]},cards:{type:'cards',items:[]},callout:{type:'callout',tone:'info',heading:'',body:''},steps:{type:'steps',heading:'',items:[]}};
