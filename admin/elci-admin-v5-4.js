(() => {
  ...
  const BLOG_CATEGORY_RULES = [
    {category:'Koruyucu Sağlık',keywords:['aşı','aşılama','kuduz','karma aşı','lösemi','bronşin','parazit','kene','pire','mikroçip','pasaport','check-up','koruyucu sağlık','hatırlatma dozu']},
    {category:'Cerrahi ve Operasyonlar',keywords:['kısırlaştırma','sterilizasyon','ameliyat','operasyon','cerrahi','anestezi','yara','dikiş','ortopedi','kırık','çıkık','fizik tedavi','rehabilitasyon']},
    {category:'Ağız ve Diş Sağlığı',keywords:['diş','ağız kokusu','diş taşı','tartar','gingivit','diş eti','periodontal','diş çekimi']},
    {category:'Acil Durumlar',keywords:['acil','ilk yardım','zehirlenme','travma','kanama','nöbet','solunum güçlüğü','sıcak çarpması','hipotermi','bilinç kaybı']},
    {category:'Beslenme ve Günlük Bakım',keywords:['beslenme','mama','kilo','obezite','su tüketimi','tüy bakımı','tırnak bakımı','günlük bakım','mama geçişi']},
    {category:'Davranış',keywords:['davranış','stres','agresyon','anksiyete','tuvalet','kedi taşıma','köpek gezdirme','korku','sosyalleşme']},
    {category:'Mevsimsel Sağlık',keywords:['mevsim','yaz bakımı','kış bakımı','sıcak hava','soğuk hava','seyahat','tatil','serin kalma']},
    {category:'Yavru ve Yaşlı Bakımı',keywords:['yavru','yaşlı','geriatrik','neonatal','yeni doğan','senior']},
    {category:'Klinik Duyuruları',keywords:['duyuru','çalışma saatleri','bayram','kliniğimiz','web sitesi','yeni hizmet','kampanya']},
    {category:'İç Hastalıkları',keywords:['diyabet','tiroid','cushing','addison','böbrek','karaciğer','idrar yolu','kalp','solunum','sindirim','kusma','ishal','iştahsızlık','ateş','hormon','kan tahlili','hemogram','biyokimya','laboratuvar','göz','kornea','deri','alerji','mantar','kulak','gebelik','doğum','jinekoloji']},
  ];
  const TAG_SYNONYMS = {
    'Aşı':['aşı','aşılama'], 'Karma Aşı':['karma aşı'], 'Kuduz Aşısı':['kuduz'], 'Lösemi Aşısı':['lösemi'],
    'İç Parazit':['iç parazit'], 'Dış Parazit':['dış parazit'], 'Diş Taşı':['diş taşı','tartar'],
    'Kısırlaştırma':['kısırlaştırma','sterilizasyon'], 'Ameliyat Sonrası':['ameliyat sonrası','operasyon sonrası'],
    'Sıcak Çarpması':['sıcak çarpması'], 'İlk Yardım':['ilk yardım'], 'Kan Tahlili':['kan tahlili'],
    'Kedi':['kedi','kediler'], 'Köpek':['köpek','köpekler'], 'Kedi ve Köpek':['kedi ve köpek'],
  };
  ...
  const BLOG_FONT_OPTIONS = [
    {label:'Manrope — modern ve kurumsal',value:'manrope'},
    {label:'Lora — sıcak ve editoryal',value:'lora'},
    {label:'Source Serif 4 — sade ve prestijli',value:'sourceSerif'},
    {label:'Merriweather — güçlü ve okunaklı',value:'merriweather'},
    {label:'Cormorant Garamond — zarif ve klasik',value:'cormorant'},
    {label:'Playfair Display — dergi tarzı',value:'playfair'},
    {label:'Georgia — klasik sistem yazı tipi',value:'georgia'},
  ];
  const BLOG_FONT_STACKS = {
    manrope:'"Manrope",-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif',lora:'"Lora",Georgia,serif',
    sourceSerif:'"Source Serif 4",Georgia,serif',merriweather:'"Merriweather",Georgia,serif',
    cormorant:'"Cormorant Garamond",Georgia,serif',playfair:'"Playfair Display",Georgia,serif',georgia:'Georgia,"Times New Roman",serif'
  };
  const DEFAULT_BLOG_DESIGN = {titleFont:'manrope',bodyFont:'manrope',cardTitleFont:'manrope',cardBodyFont:'manrope'};
  const safeBlogFont = value => Object.hasOwn(BLOG_FONT_STACKS,String(value||'')) ? String(value) : 'manrope';
  const normalizeBlogDesign = value => ({titleFont:safeBlogFont(value?.titleFont),bodyFont:safeBlogFont(value?.bodyFont),cardTitleFont:safeBlogFont(value?.cardTitleFont),cardBodyFont:safeBlogFont(value?.cardBodyFont)});
  const fontSelectOptions = (withDefault=false,defaultLabel='Site varsayılanını kullan') => [...(withDefault?[{label:defaultLabel,value:'default'}]:[]),...BLOG_FONT_OPTIONS];
  ...
  const state = {
    ...
    collectionFilters:{}, calendarIdeaOffset:0, saving:false, blogDesign:clone(DEFAULT_BLOG_DESIGN), blogDesignSha:null,
  };
  ...
  function blogDraftText(data={}) {
    const sections=(data.editorialSections||[]).flatMap(block=>{
      if(!block||typeof block!=='object')return[];
      const values=[block.heading,block.body,block.caption,block.alt];
      if(Array.isArray(block.items))values.push(...block.items.map(item=>typeof item==='string'?item:item?.item||''));
      if(Array.isArray(block.images))values.push(...block.images.flatMap(item=>[item?.alt||'',item?.caption||'']));
      if(block.type==='cards'&&Array.isArray(block.items))values.push(...block.items.flatMap(item=>[item?.title||'',item?.body||'',item?.alt||'']));
      return values;
    });
    return [data.title,data.summary,data.content,data.species,...(data.tags||[]),...sections].filter(Boolean).join(' ');
  }
  function termCount(text,term){
    const hay=normalize(text),needle=normalize(term);if(!needle)return 0;
    let count=0,start=0;while((start=hay.indexOf(needle,start))>-1&&count<4){count++;start+=needle.length;}return count;
  }
  function inferBlogCategory(data={}) {
    const title=String(data.title||''),summary=String(data.summary||''),body=blogDraftText({...data,title:'',summary:''});
    const selectedTags=Array.isArray(data.tags)?data.tags:[];
    let best={category:'',score:0,reasons:[]};
    BLOG_CATEGORY_RULES.forEach(rule=>{
      let score=0;const reasons=[];
      rule.keywords.forEach(keyword=>{
        const titleHits=termCount(title,keyword),summaryHits=termCount(summary,keyword),bodyHits=termCount(body,keyword);
        const tagHit=selectedTags.some(tag=>normalize(tag).includes(normalize(keyword))||normalize(keyword).includes(normalize(tag)));
        const value=titleHits*9+summaryHits*5+bodyHits*2+(tagHit?10:0);
        if(value){score+=value;if(reasons.length<3)reasons.push(keyword);}
      });
      if(score>best.score)best={category:rule.category,score,reasons};
    });
    if(!best.score){
      const text=normalize([title,summary,body].join(' '));
      const hasCat=/\bkedi|kediler|yavru kedi/.test(text),hasDog=/\bköpek|köpekler|yavru köpek/.test(text);
      if(hasCat&&!hasDog)best={category:'Kedi Sağlığı',score:2,reasons:['kedi']};
      else if(hasDog&&!hasCat)best={category:'Köpek Sağlığı',score:2,reasons:['köpek']};
      else if(text.trim())best={category:'İç Hastalıkları',score:1,reasons:['genel sağlık içeriği']};
    }
    return {...best,confidence:best.score>=18?'Yüksek':best.score>=7?'Orta':best.score?'Temel':'Bekleniyor'};
  }
  function inferBlogTags(data={}) {
    const title=normalize(data.title),summary=normalize(data.summary),all=normalize(blogDraftText(data));
    const category=String(data.category||inferBlogCategory(data).category||'');
    const generic=new Set(['sağlığı','bakımı','tedavi','muayene','kontrol']);
    const scored=BLOG_TAGS.map(tag=>{
      const phrases=unique([tag,...(TAG_SYNONYMS[tag]||[])]).map(normalize);let score=0;
      phrases.forEach(phrase=>{
        if(!phrase||generic.has(phrase))return;
        if(title.includes(phrase))score+=9;
        if(summary.includes(phrase))score+=5;
        if(all.includes(phrase))score+=2;
      });
      if(category==='Koruyucu Sağlık'&&['Aşı','İç Parazit','Dış Parazit','Check-up'].includes(tag))score+=1;
      if(category==='Acil Durumlar'&&['Acil Belirti','İlk Yardım'].includes(tag))score+=1;
      return{tag,score};
    }).filter(item=>item.score>0).sort((a,b)=>b.score-a.score||b.tag.length-a.tag.length);
    return unique(scored.map(item=>item.tag)).slice(0,8);
  }
  function currentBlogSnapshot(){
    const form=$('#contentForm');if(!form)return clone(state.edit?.data||{});
    const values=name=>form.elements[name]?.value||'';
    const blockText=$$('.editorial-block input[type="text"],.editorial-block textarea').map(node=>node.value).join(' ');
    return{
      title:values('title'),summary:values('summary'),content:values('content'),species:values('species')||'Genel',
      tags:$$('input[name="tags"]:checked').map(node=>node.value),editorialSections:[{body:blockText}],
      category:values('category')||'',categoryMode:values('categoryMode')||'auto'
    };
  }
  ...
  function confirmAction({title='İşlemi onayla',message='',action='confirm',contentId=''}) {
    return new Promise(resolve => {
      openModal({title,kicker:'İŞLEM ONAYI',body:`<p>${esc(message)}</p>`,footer:`<button type="button" class="button" data-action="cancel" data-content-id="${attr(contentId)}">İptal</button><button type="button" class="button danger" data-action="${attr(action)}" data-content-id="${attr(contentId)}">Onayla</button>`});
      const done=value=>{closeModal();resolve(value);};
      $('#modalFooter [data-action="cancel"]').addEventListener('click',()=>done(false),{once:true});
      $('#modalFooter [data-action="'+action+'"]').addEventListener('click',event=>{event.currentTarget.disabled=true;done(true);},{once:true});
    });
  }
  ...
  async function loadBlogDesign(force=false){
    const file=await readJson('settings/blog-design.json',DEFAULT_BLOG_DESIGN,force);
    state.blogDesign=normalizeBlogDesign(file.data||DEFAULT_BLOG_DESIGN);state.blogDesignSha=file.sha||null;return state.blogDesign;
  }
  function resolvePreviewDesign(data={}){
    const base=normalizeBlogDesign(state.blogDesign||DEFAULT_BLOG_DESIGN),local=data.design||{};
    const pick=(value,fallback)=>value&&value!=='default'?safeBlogFont(value):fallback;
    return {titleFont:pick(local.titleFont,base.titleFont),bodyFont:pick(local.bodyFont,base.bodyFont),cardTitleFont:pick(local.cardTitleFont,base.cardTitleFont),cardBodyFont:pick(local.cardBodyFont,base.cardBodyFont)};
  }
  ...
  async function renderEditor(name, slug) {
    ...
    if(name==='blog')await loadBlogDesign();
    let item = null;
  ...
  function defaultItem(name) {
    if (name==='blog') {
      ...
      return {published:true,featured:false,title:idea?.title||'',date:nowIso(),unpublishAt:'',category:idea?.category||'',categoryMode:'auto',species:idea?.species||'Genel',tags:Array.isArray(idea?.tags)?idea.tags.slice(0,8):[],relatedService:idea?.relatedService||'',summary:idea?.summary||'',cover:'',coverCaption:'',design:{titleFont:'default',bodyFont:'default',cardTitleFont:'default',cardBodyFont:'default'},contentMode:idea?.content?'standard':'',content:idea?.content||'',editorialSections:[],advanced:{slug:'',seoTitle:'',seoDescription:'',youtubeId:'',author:'Elçi Veteriner Kliniği'}};
    }
  ...
  function blogContentMode(data) {
    ...
    const hasText=!!String(data?.content||'').trim();
    const hasBlocks=Array.isArray(data?.editorialSections)&&data.editorialSections.length>0;
    if(hasText&&!hasBlocks)return'standard';
    if(hasBlocks&&!hasText)return'visual';
    return'';
  }
  function contentModeSelector(mode) {
    return `<section class="form-card content-mode-card" id="contentModeCard"><div class="section-heading"><span class="section-icon"><i class="fa-solid fa-wand-magic-sparkles"></i></span><div><h2>Önce yazının yapısını seçin</h2><p>Metni yazmadan önce biçimi seçin. Böylece gereksiz alanlar açılmaz ve görsel düzen bozulmaz.</p></div></div><div class="content-mode-grid"><label class="content-mode-option ${mode==='standard'?'selected':''}"><input type="radio" name="contentMode" value="standard" ${mode==='standard'?'checked':''}><span class="mode-icon"><i class="fa-solid fa-align-left"></i></span><span><strong>Sadece metin</strong><small>Tek metin kutusu kullanılır. Hazır metni hızla yapıştırmak için.</small></span><i class="fa-solid fa-circle-check mode-check"></i></label><label class="content-mode-option ${mode==='visual'?'selected':''}"><input type="radio" name="contentMode" value="visual" ${mode==='visual'?'checked':''}><span class="mode-icon"><i class="fa-regular fa-images"></i></span><span><strong>Görselli ve bölümlü yazı</strong><small>Metin, girintili görsel, görsel+metin, galeri, bilgi kutusu ve adımlar.</small></span><i class="fa-solid fa-circle-check mode-check"></i></label></div>${mode?'':`<div class="mode-required-note"><i class="fa-solid fa-arrow-up"></i><span>Devam etmek için yukarıdaki iki yapıdan birini seçin.</span></div>`}</section>`;
  }
  function standardContentSection(data) {
    return `<section class="form-card"><div class="section-heading"><span class="section-icon"><i class="fa-solid fa-align-left"></i></span><div><h2>Yazının metni</h2><p>Bu biçimde yalnızca aşağıdaki metin alanı kullanılır. Kapak görseli üstte ayrıca görünür.</p></div></div><div class="field full"><label for="f-content">Metin</label><div class="rich-toolbar"><button type="button" data-format="h2">Büyük başlık</button><button type="button" data-format="h3">Alt başlık</button><button type="button" data-format="bold">Kalın</button><button type="button" data-format="list">Madde</button><button type="button" data-format="number">Numara</button><button type="button" data-format="quote">Bilgi notu</button></div><textarea class="rich-text" id="f-content" name="content" placeholder="Hazır metni buraya yapıştırın…">${esc(data.content||'')}</textarea><small>Görseli metnin arasına koymak istiyorsanız “Görselli ve bölümlü yazı” seçin.</small></div></section>`;
  }
  function visualContentSection(data) {
    const blocks=data.editorialSections||[];
    return `<section class="form-card editorial-card"><div class="item-card-head"><div><h2 style="margin:0">Yazı bölümleri</h2><small style="color:var(--muted)">Bölüm türünü seçin; yalnızca seçtiğiniz bölümün alanları açılır.</small></div><strong class="block-count">${blocks.length} bölüm</strong></div><div class="editorial-help"><strong>Estetik düzen nasıl korunur?</strong><span>Yazı tipi, ölçü, renk ve boşluklar sistem tarafından sabitlenir. Siz yalnızca içerik ve yerleşimi seçersiniz.</span><div class="editorial-help-grid"><span><i class="fa-solid fa-align-left"></i><b>Sadece metin</b> Başlık ve paragraf.</span><span><i class="fa-regular fa-image"></i><b>Tek görsel</b> Geniş veya girintili.</span><span><i class="fa-solid fa-table-columns"></i><b>Görsel + metin</b> Sağda veya solda.</span><span><i class="fa-solid fa-grip"></i><b>Görsel galerisi</b> 2–4 görsel birlikte.</span><span><i class="fa-regular fa-rectangle-list"></i><b>Öne çıkan kutucuklar</b> Yalnızca gerektiğinde 1–3 prestijli bilgi kartı.</span></div></div><div class="block-builder" id="editorialBuilder"><div class="block-toolbar"><button type="button" class="button" data-add-block="text"><i class="fa-solid fa-align-left"></i> Metin</button><button type="button" class="button" data-add-block="image"><i class="fa-regular fa-image"></i> Tek görsel</button><button type="button" class="button" data-add-block="split"><i class="fa-solid fa-table-columns"></i> Görsel + metin</button><button type="button" class="button" data-add-block="gallery"><i class="fa-solid fa-grip"></i> Galeri</button><button type="button" class="button" data-add-block="cards"><i class="fa-regular fa-rectangle-list"></i> Kutucuklar</button><button type="button" class="button" data-add-block="callout"><i class="fa-solid fa-circle-info"></i> Bilgi kutusu</button><button type="button" class="button" data-add-block="steps"><i class="fa-solid fa-list-ol"></i> Adımlar</button></div><div id="editorialBlocks">${blocks.map((block,index)=>renderEditorialBlock(block,index,blocks.length)).join('')}</div><p class="editorial-empty ${blocks.length?'hidden':''}" id="editorialEmpty"><i class="fa-regular fa-square-plus"></i> İlk bölümünüzü yukarıdaki düğmelerden seçin.</p></div></section>`;
  }

  function autoCategoryField(data){
  ...
  function readabilitySummary(data){
    ...
    const headings=(text.match(/^#{2,3}\s+/gm)||[]).length+(data.editorialSections||[]).filter(block=>block?.heading).length;
    const checks=[
      {ok:String(data.title||'').trim().length>=24&&String(data.title||'').trim().length<=85,label:'Başlık açık ve dengeli'},
      {ok:String(data.summary||'').trim().length>=55&&String(data.summary||'').trim().length<=220,label:'Kısa açıklama okunaklı'},
      {ok:words>=35,label:'Yeterli içerik var'},
      {ok:words<220||headings>0,label:'Uzun metin bölümlere ayrılmış'},
    ];
    return `<div class="readability-checks">${checks.map(item=>`<span class="${item.ok?'ok':'wait'}"><i class="fa-solid ${item.ok?'fa-check':'fa-minus'}"></i>${esc(item.label)}</span>`).join('')}</div>`;
  }
  function previewHasMedia(data){
    return (data.editorialSections||[]).some(block=>{
  ...
  function previewReadingMinutes(data){
    const raw=[data.title,data.summary,data.content,...(data.editorialSections||[]).flatMap(block=>[block?.heading,block?.body,...(block?.items||[]).flatMap(item=>[typeof item==='string'?item:item?.item,item?.title,item?.body])])].filter(Boolean).join(' ').replace(/<[^>]*>/g,' ').replace(/\s+/g,' ').trim();
    return Math.max(1,Math.ceil(Math.max(raw.split(' ').filter(Boolean).length,Math.ceil(raw.length/6))/190));
  }
  function buildBlogPreviewHtml(data){
    ...
    const showCover=!!data.cover&&(mode!=='visual'||!previewHasMedia(data));
    return `<article class="health-article-preview prestige-preview" style="--preview-title-font:${attr(BLOG_FONT_STACKS[resolvePreviewDesign(data).titleFont])};--preview-body-font:${attr(BLOG_FONT_STACKS[resolvePreviewDesign(data).bodyFont])};--preview-card-title-font:${attr(BLOG_FONT_STACKS[resolvePreviewDesign(data).cardTitleFont])};--preview-card-body-font:${attr(BLOG_FONT_STACKS[resolvePreviewDesign(data).cardBodyFont])}"><div class="health-preview-breadcrumb">Ana Sayfa <i class="fa-solid fa-chevron-right"></i> Blog <i class="fa-solid fa-chevron-right"></i> Ön izleme</div><header><div class="health-preview-topline"><div><span class="health-kicker"><i class="fa-regular fa-bookmark"></i>${esc(category)}</span><span class="health-category"><i class="fa-regular fa-user"></i>${esc(data.advanced?.author||'Elçi Veteriner Kliniği')}</span></div><span class="health-read-time"><i class="fa-regular fa-clock"></i>${previewReadingMinutes(data)} dk okuma</span></div><h1>${esc(data.title||'Yazı başlığınız burada görünecek')}</h1><div class="health-title-divider"><span></span><i class="fa-solid fa-paw"></i><span></span></div>${data.summary?`<p class="health-lead">${esc(data.summary)}</p>`:''}</header>${showCover?`<figure class="health-preview-cover"><img src="${attr(data.cover)}" alt="">${data.coverCaption?`<figcaption>${esc(data.coverCaption)}</figcaption>`:''}</figure>`:''}<div class="health-preview-body">${mode==='visual'?editorialPreviewHtml(data):simpleRichPreview(data.content||'<p>Metninizi yazdıkça ön izleme burada güncellenecek.</p>')}</div><aside class="health-disclaimer"><i class="fa-solid fa-leaf"></i><div><strong>Unutmayın</strong><span>Her patinin ihtiyacı farklıdır. Bu içerik genel bilgilendirme amaçlıdır.</span></div></aside></article>`;
  }
  ...
  function blogTypographySection(data){
    const design=data.design||{};const defaults=state.blogDesign||DEFAULT_BLOG_DESIGN;
    const defaultName=key=>BLOG_FONT_OPTIONS.find(x=>x.value===defaults[key])?.label||'Manrope';
    return `<section class="form-card blog-typography-card"><div class="section-heading"><span class="section-icon"><i class="fa-solid fa-font"></i></span><div><h2>Bu yazının yazı tipleri</h2><p>Seçim yapmazsanız sizin sabitlediğiniz site varsayılanları kullanılır.</p></div></div><div class="form-grid">${selectField('design.titleFont','Başlık yazı tipi',design.titleFont||'default',fontSelectOptions(true,`Site varsayılanı — ${defaultName('titleFont')}`),'',true)}${selectField('design.bodyFont','Yazı metni yazı tipi',design.bodyFont||'default',fontSelectOptions(true,`Site varsayılanı — ${defaultName('bodyFont')}`),'',true)}</div><small>Kalıcı varsayılanları “Ayarlar ve güvenlik → Blog tasarım varsayılanları” bölümünden siz belirlersiniz.</small></section>`;
  }
  ...
  function blogForm(data) {
    ...
    return `<section class="form-card"><div class="section-heading"><span class="section-icon"><i class="fa-solid fa-paper-plane"></i></span><div><h2>Yayın ayarları</h2><p>Şimdi yayınlayabilir, ileri tarihe planlayabilir veya taslak bırakabilirsiniz.</p></div></div><div class="form-grid">${switchField('published','Yayında',data.published!==false,'Kapattığınızda yazı panelde kalır, siteden kaldırılır.')}${switchField('featured','Ana sayfada öne çıkar',!!data.featured,'Ana sayfada en fazla üç yazı gösterilir.')}${dateTimeField('date','Yayınlama zamanı',toInputDateTime(data.date),'Şimdi yayınla düğmesi bu alanı otomatik olarak günceller.',{now:true})}${dateTimeField('unpublishAt','Otomatik yayından kalkma',toInputDateTime(data.unpublishAt),'Süresiz kalacaksa boş bırakın. Bitiş zamanı yayın zamanından sonra olmalıdır.',{clear:true})}</div></section>
      <section class="form-card"><div class="section-heading"><span class="section-icon"><i class="fa-solid fa-file-pen"></i></span><div><h2>Yazının temel bilgileri</h2><p>Başlığı ve kısa açıklamayı yazın; ana kategori sistem tarafından belirlenir.</p></div></div><div class="form-grid">${inputField('title','Başlık',data.title,'text','Okuyucunun aradığı konuyu açıkça anlatan başlık.',true,'required maxlength="120"')}${autoCategoryField(data)}${selectField('species','Hayvan türü',data.species||'Genel',SPECIES,'Kategori otomatik belirlenirken hayvan türü de dikkate alınır.')}${textField('summary','Kartta görünen kısa açıklama',data.summary,'1–3 kısa ve samimi cümle. Okuyucunun yazıyı açma kararını kolaylaştırır.',true,3)}${imageField('cover','Kapak görseli',data.cover,'assets/img/uploads/blog','Blog kartında kullanılır. Sadece metin yazısında içerikte de görünür; görselli yazıda tekrar edilmez.')}${inputField('coverCaption','Kapak görseli alt yazısı (isteğe bağlı)',data.coverCaption||'','text','Görselin altında küçük puntoda görünür.',true)}</div></section>
      ${blogTypographySection(data)}
      ${tagSelector(data.tags||[])}
      ${contentModeSelector(mode)}
      ${mode==='visual'?visualContentSection(data):mode==='standard'?standardContentSection(data):`<section class="form-card mode-waiting"><i class="fa-solid fa-arrow-up"></i><h2>Yazı yapısını seçin</h2><p>Seçiminizden sonra yalnızca kullanacağınız içerik alanları açılacak.</p></section>`}
      ${blogInlinePreview(data)}
      <details class="advanced"><summary>SEO ve gelişmiş ayarlar</summary><div class="advanced-body"><div class="form-grid">${selectField('relatedService','Yazı sonunda önerilecek hizmet (isteğe bağlı)',data.relatedService||'',[{label:'Hizmet önerme',value:''},...RELATED_SERVICES],'Yazıyı Hizmetler sayfasına taşımaz; yalnızca yazının sonunda bağlantı önerir.',true)}${inputField('advanced.slug','Özel bağlantı adı',adv.slug||'','text','Boş bırakırsanız başlıktan otomatik oluşturulur.',true)}${inputField('advanced.seoTitle','Google başlığı',adv.seoTitle||'','text','Boşsa yazı başlığı kullanılır.',true)}${textField('advanced.seoDescription','Google açıklaması',adv.seoDescription||'','Boşsa kısa açıklama kullanılır.',true,3)}${inputField('advanced.youtubeId','YouTube video kimliği',adv.youtubeId||'','text','İsteğe bağlı.')}${inputField('advanced.author','Yazar',adv.author||'Elçi Veteriner Kliniği')}</div></div></details>`;
  }

  function renderEditorialBlock(block,index,total) {
    const type=block.type||'text';
    const typeLabel={text:'Sadece metin',image:'Tek görsel',split:'Görsel + metin',gallery:'Görsel galerisi',cards:'Öne çıkan kutucuklar',callout:'Bilgi kutusu',steps:'Numaralı adımlar'}[type]||'Yazı bölümü';
    let fields='';
    ...
    if(type==='image') fields=`${blockImageField(`block-image-${index}`,block.image||'','Görsel')}${inputField(`block.${index}.alt`,'Görsel açıklaması',block.alt||'','text','Google ve erişilebilirlik için kısa açıklama.',true)}${inputField(`block.${index}.caption`,'Görsel alt yazısı (isteğe bağlı)',block.caption||'','text','',true)}${selectField(`block.${index}.size`,'Sunum biçimi',block.size||'wide',[{label:'Geniş',value:'wide'},{label:'Girintili / ortalı',value:'compact'}])}${selectField(`block.${index}.fit`,'Görsel görünümü',block.fit||'contain',[{label:'Tamamını göster — afiş / belge / infografik',value:'contain'},{label:'Alanı doldur — fotoğraf gibi kırp',value:'cover'}])}`;
    if(type==='split') fields=`${inputField(`block.${index}.heading`,'Bölüm başlığı (isteğe bağlı)',block.heading||'','text','',true)}${textField(`block.${index}.body`,'Metin (isteğe bağlı)',block.body||'','Yazı görselin yanından başlayıp görsel bittikten sonra tam genişlikte devam eder.',true,8)}${blockImageField(`block-image-${index}`,block.image||'','Görsel (isteğe bağlı)')}${inputField(`block.${index}.alt`,'Görsel açıklaması',block.alt||'','text','Erişilebilirlik için kısa açıklama.',true)}${inputField(`block.${index}.caption`,'Görsel alt yazısı (isteğe bağlı)',block.caption||'','text','Görselin altında küçük puntoda görünür.',true)}${selectField(`block.${index}.imageSide`,'Görsel konumu',block.imageSide||'right',[{label:'Sağda',value:'right'},{label:'Solda',value:'left'}])}${selectField(`block.${index}.fit`,'Görsel görünümü',block.fit||'cover',[{label:'Fotoğraf gibi kırp',value:'cover'},{label:'Tamamını göster',value:'contain'}])}`;
    if(type==='gallery') fields=`${selectField(`block.${index}.layout`,'Galeri düzeni',block.layout||'auto',[{label:'Otomatik',value:'auto'},{label:'İki sütun',value:'two'},{label:'Büyük görsel + küçükler',value:'feature'}],'Yalnızca yüklenen görseller gösterilir.',true)}${selectField(`block.${index}.fit`,'Görsellerin görünümü',block.fit||'cover',[{label:'Fotoğraf gibi kırp',value:'cover'},{label:'Tamamını göster — afiş / belge',value:'contain'}],'',true)}<div class="gallery-editor-grid">${[0,1,2,3].map(slot=>galleryImageField(index,slot,(block.images||[])[slot]||{})).join('')}</div>`;
    if(type==='cards') fields=`<div class="field full"><small>Bu bölüm zorunlu değildir. En fazla üç kutucuk eklenir; yalnızca doldurduklarınız yayımlanır. Renkler sitenin mevcut tema renklerinden otomatik alınır.</small></div>${selectField(`block.${index}.titleFont`,'Kutucuk başlık yazı tipi',block.titleFont||'default',fontSelectOptions(true,'Blog kutu varsayılanını kullan'))}${selectField(`block.${index}.bodyFont`,'Kutucuk metin yazı tipi',block.bodyFont||'default',fontSelectOptions(true,'Blog kutu varsayılanını kullan'))}<div class="feature-card-editor-grid">${[0,1,2].map(slot=>featureCardField(index,slot,(block.items||[])[slot]||{})).join('')}</div>`;
    ...
    return `<article class="editorial-block" data-block-index="${index}" data-block-type="${attr(type)}"><header><div><span class="block-number">${index+1}</span><strong>${esc(typeLabel)}</strong></div><div class="block-actions"><button type="button" class="icon-button" data-move-block="-1" data-block="${index}" ${index===0?'disabled':''} aria-label="Yukarı taşı"><i class="fa-solid fa-arrow-up"></i></button><button type="button" class="icon-button" data-move-block="1" data-block="${index}" ${index===total-1?'disabled':''} aria-label="Aşağı taşı"><i class="fa-solid fa-arrow-down"></i></button><button type="button" class="icon-button" data-duplicate-block="${index}" aria-label="Bölümü çoğalt"><i class="fa-regular fa-copy"></i></button><button type="button" class="icon-button remove-block" data-remove-block="${index}" aria-label="Bölümü kaldır"><i class="fa-solid fa-trash"></i></button></div></header><div class="form-grid">${fields}</div></article>`;
  }
  function blockImageField(key,value,label) {
    return `<div class="field full"><label>${esc(label)}</label><div class="image-picker"><div class="image-preview" data-image-preview="${attr(key)}">${value?`<img src="${attr(value)}" alt="">`:'<i class="fa-regular fa-image"></i>'}</div><div><input type="hidden" name="${attr(key)}" value="${attr(value)}"><div class="button-row"><button type="button" class="button" data-pick-image="${attr(key)}" data-folder="assets/img/uploads/blog"><i class="fa-solid fa-upload"></i> Görsel yükle</button>${value?`<button type="button" class="button danger" data-clear-image="${attr(key)}">Kaldır</button>`:''}</div></div></div></div>`;
  }
  function galleryImageField(index,slot,item={}) {
    const key=`block-gallery-${index}-${slot}`;
    return `<div class="gallery-editor-item"><strong>${slot+1}. görsel</strong>${blockImageField(key,item.image||'',`Görsel ${slot+1}`)}${inputField(`block.${index}.gallery.${slot}.alt`,'Görsel açıklaması',item.alt||'','text','',true)}${inputField(`block.${index}.gallery.${slot}.caption`,'Alt yazı (isteğe bağlı)',item.caption||'','text','',true)}</div>`;
  }

  function featureCardField(index,slot,item={}) {
    const key=`block-card-${index}-${slot}`;
    const icons=[
      {label:'Pati',value:'paw'},{label:'Kalkan',value:'shield'},{label:'Aşı',value:'syringe'},
      {label:'Yaprak',value:'leaf'},{label:'Ev',value:'home'},{label:'Kalp',value:'heart'},
      {label:'Beslenme',value:'bowl'},{label:'Yıldız',value:'star'}
    ];
    return `<div class="feature-card-editor"><strong>${slot+1}. kutucuk</strong><div class="form-grid">${selectField(`block.${index}.card.${slot}.icon`,'Simge',item.icon||'paw',icons)}${selectField(`block.${index}.card.${slot}.position`,'Konum',item.position||'auto',[{label:'Otomatik',value:'auto'},{label:'Sol',value:'left'},{label:'Orta',value:'center'},{label:'Sağ',value:'right'},{label:'Tam satır',value:'full'}])}${selectField(`block.${index}.card.${slot}.size`,'Kutu büyüklüğü',item.size||'medium',[{label:'Küçük',value:'small'},{label:'Orta',value:'medium'},{label:'Büyük',value:'large'},{label:'Tam genişlik',value:'full'}])}${selectField(`block.${index}.card.${slot}.textSize`,'Metin büyüklüğü',item.textSize||'normal',[{label:'Küçük',value:'small'},{label:'Normal',value:'normal'},{label:'Büyük',value:'large'}])}${inputField(`block.${index}.card.${slot}.title`,'Başlık',item.title||'','text','',true)}${textField(`block.${index}.card.${slot}.body`,'Kısa metin',item.body||'','Metin kutudan taşmaz; gerektiğinde kutu aşağı doğru büyür.',true,4)}${blockImageField(key,item.image||'','Görsel (isteğe bağlı)')}${inputField(`block.${index}.card.${slot}.alt`,'Görsel açıklaması',item.alt||'','text','',true)}</div></div>`;
  }
  ...
  function bindEditor(name) {
    ...
    $$('[data-move-block]').forEach(button => button.addEventListener('click',()=>moveEditorialBlock(Number(button.dataset.block),Number(button.dataset.moveBlock))));
    $$('[data-duplicate-block]').forEach(button => button.addEventListener('click',()=>duplicateEditorialBlock(Number(button.dataset.duplicateBlock))));
    bindDateHelpers();
    if(name==='blog'){bindTagSelector();bindContentModeSelector();bindAutoCategory();bindBlogInlinePreview();}
  ...
  function bindTagSelector(){
    const search=$('#tagSearch'),options=$('#tagOptions'),count=$('#tagCount'),selected=$('#selectedTags'),empty=$('#tagEmpty'),suggestionBox=$('#tagSuggestions'),suggestionChips=$('#tagSuggestionChips');if(!options)return;
    ...
    const filter=()=>{const q=normalize(search?.value);let visible=0;$$('.choice',options).forEach(label=>{const show=!q||label.dataset.tagLabel.includes(q);label.hidden=!show;if(show)visible++;});empty.classList.toggle('hidden',visible>0);};
    const showSuggestions=()=>{
      const suggestions=inferBlogTags(currentBlogSnapshot()).filter(tag=>!boxes().some(box=>box.checked&&box.value===tag)).slice(0,6);
      suggestionBox.hidden=!suggestions.length;
      suggestionChips.innerHTML=suggestions.map(tag=>`<button type="button" class="suggested-tag" data-suggest-tag="${attr(tag)}"><i class="fa-solid fa-plus"></i>${esc(tag)}</button>`).join('');
      $$('[data-suggest-tag]',suggestionChips).forEach(button=>button.addEventListener('click',()=>{const box=boxes().find(x=>x.value===button.dataset.suggestTag);if(box&&!box.checked){if(boxes().filter(x=>x.checked).length>=8){toast('En fazla 8 etiket','Bir etiketi kaldırıp tekrar deneyin.','warning');return;}box.checked=true;box.dispatchEvent(new Event('change',{bubbles:true}));showSuggestions();}}));
      if(!suggestions.length)toast('Yeni öneri yok','Başlık veya metin biraz daha ayrıntılı olduğunda yeni etiketler önerebiliriz.','warning');
    };
    ...
    $('#tagSearchButton')?.addEventListener('click',filter);
    $('#suggestTagsButton')?.addEventListener('click',showSuggestions);
    $('#clearTags')?.addEventListener('click',()=>{boxes().forEach(x=>x.checked=false);update();});
    filter();update();
  }
  function bindAutoCategory(){
    const form=$('#contentForm');if(!form)return;
    let timer=null;
    const schedule=()=>{clearTimeout(timer);timer=setTimeout(updateAutoCategory,180);};
    form.addEventListener('input',event=>{if(event.target.matches('[name="title"],[name="summary"],[name="content"],.editorial-block input[type="text"],.editorial-block textarea'))schedule();});
    form.addEventListener('change',event=>{if(event.target.matches('[name="species"],input[name="tags"]'))schedule();});
    $('#useManualCategory')?.addEventListener('click',()=>{const select=$('#manualCategorySelect'),hidden=form.elements.category,mode=form.elements.categoryMode;if(select&&hidden&&mode){hidden.value=select.value;mode.value='manual';updateAutoCategory();toast('Kategori sabitlendi',select.value);}});
    $('#useAutoCategory')?.addEventListener('click',()=>{const mode=form.elements.categoryMode;if(mode){mode.value='auto';updateAutoCategory();toast('Otomatik kategori açıldı');}});
    updateAutoCategory();
  }
  function updateAutoCategory(){
    const form=$('#contentForm');if(!form||state.edit?.name!=='blog')return;
    const hidden=form.elements.category,mode=form.elements.categoryMode;if(!hidden||!mode)return;
    const snapshot=currentBlogSnapshot(),inference=inferBlogCategory(snapshot);
    if(mode.value!=='manual')hidden.value=inference.category||'';
  ...
  function bindBlogInlinePreview(){
    const stage=$('#inlinePreviewStage'),target=$('#inlineBlogPreview'),quality=$('#writingQuality'),form=$('#contentForm');if(!stage||!target||!form)return;
    let timer=null;
    const refresh=()=>{updateAutoCategory();const data=collectCurrentForm();target.innerHTML=buildBlogPreviewHtml(data);quality.innerHTML=readabilitySummary(data);};
    const schedule=()=>{clearTimeout(timer);timer=setTimeout(refresh,260);};
    form.addEventListener('input',schedule);form.addEventListener('change',schedule);
    $('#refreshInlinePreview')?.addEventListener('click',refresh);
    $$('[data-preview-device]').forEach(button=>button.addEventListener('click',()=>{$$('[data-preview-device]').forEach(x=>x.classList.remove('active'));button.classList.add('active');stage.dataset.device=button.dataset.previewDevice;}));
    refresh();
  }
  ...
  function syncBlocksToState() {
    ...
    state.edit.data.editorialSections=nodes.map(block=>{
      ...
      if(type==='image')return{type,image:block.querySelector(`[name="block-image-${index}"]`)?.value||'',alt:get('alt'),caption:get('caption'),size:get('size')||'wide',fit:get('fit')||'contain'};
      if(type==='split')return{type,heading:get('heading'),body:get('body'),image:block.querySelector(`[name="block-image-${index}"]`)?.value||'',alt:get('alt'),caption:get('caption'),imageSide:get('imageSide')||'right',fit:get('fit')||'cover'};
      if(type==='gallery')return{type,layout:get('layout')||'auto',fit:get('fit')||'cover',images:[0,1,2,3].map(slot=>({image:block.querySelector(`[name="block-gallery-${index}-${slot}"]`)?.value||'',alt:get(`gallery.${slot}.alt`),caption:get(`gallery.${slot}.caption`)})).filter(item=>item.image)};
      if(type==='cards')return{type,titleFont:get('titleFont')||'default',bodyFont:get('bodyFont')||'default',items:[0,1,2].map(slot=>({icon:get(`card.${slot}.icon`)||'paw',position:get(`card.${slot}.position`)||'auto',size:get(`card.${slot}.size`)||'medium',textSize:get(`card.${slot}.textSize`)||'normal',title:get(`card.${slot}.title`),body:get(`card.${slot}.body`),image:block.querySelector(`[name="block-card-${index}-${slot}"]`)?.value||'',alt:get(`card.${slot}.alt`)})).filter(item=>item.title||item.body||item.image)};
  ...
  function collectCurrentForm() {
    ...
    if(name==='blog'){
      ...
      data.categoryMode=fd.get('categoryMode')||data.categoryMode||'auto';
      if(data.categoryMode!=='manual')data.category=inferBlogCategory(data).category||data.category||'İç Hastalıkları';
    }
  ...
  function addEditorialBlock(type) {
    const current=collectCurrentForm();
    const defaults={text:{type:'text',heading:'',body:''},image:{type:'image',image:'',alt:'',caption:'',size:'wide',fit:'contain'},split:{type:'split',heading:'',body:'',image:'',alt:'',caption:'',imageSide:'right',fit:'cover'},gallery:{type:'gallery',layout:'auto',fit:'cover',images:[]},cards:{type:'cards',titleFont:'default',bodyFont:'default',items:[]},callout:{type:'callout',tone:'info',heading:'',body:''},steps:{type:'steps',heading:'',items:[]}};
    if(!defaults[type])return;
    current.editorialSections=[...(current.editorialSections||[]),clone(defaults[type])]; state.edit.data=current; rerenderEditorPreserving('bottom');
  }
  ...
  function moveEditorialBlock(index,delta){const current=collectCurrentForm(),blocks=[...(current.editorialSections||[])],target=index+delta;if(target<0||target>=blocks.length)return;[blocks[index],blocks[target]]=[blocks[target],blocks[index]];current.editorialSections=blocks;state.edit.data=current;rerenderEditorPreserving(`block-${target}`);}
  function duplicateEditorialBlock(index){const current=collectCurrentForm(),blocks=[...(current.editorialSections||[])];if(!blocks[index])return;blocks.splice(index+1,0,clone(blocks[index]));current.editorialSections=blocks;state.edit.data=current;rerenderEditorPreserving(`block-${index+1}`);}
  function rerenderEditorPreserving(focus='top'){main.innerHTML=editorShell(state.edit.name,state.edit.data,state.edit.item);bindEditor(state.edit.name);requestAnimationFrame(()=>{if(focus==='bottom')$('#editorialBlocks')?.lastElementChild?.scrollIntoView({behavior:'smooth',block:'center'});else if(String(focus).startsWith('block-'))$$('.editorial-block')[Number(focus.split('-')[1])]?.scrollIntoView({behavior:'smooth',block:'center'});else window.scrollTo({top:0,behavior:'smooth'});});}
  ...
  function validateEditor(name,data,action) {
    ...
    if(name==='blog'&&!isDraft){
      const mode=blogContentMode(data);
      if(!mode)return'Önce “Sadece metin” veya “Görselli ve bölümlü yazı” yapısını seçin.';
      ...
      if(mode==='visual'){
        const meaningful=(data.editorialSections||[]).some(block=>{
          ...
          if(block.type==='gallery')return Array.isArray(block.images)&&block.images.filter(item=>item?.image).length>=2;
          if(block.type==='cards')return Array.isArray(block.items)&&block.items.some(item=>String(item?.title||item?.body||item?.image||'').trim());
        ...
        if(!meaningful)return'Görselli yazıda en az bir dolu bölüm ekleyin. Galeri kullanıyorsanız en az iki görsel yükleyin.';
      }
    }
  ...
  function editorialPreviewHtml(data){
    const sections=(data.editorialSections||[]).map(block=>{
      ...
      if(block.type==='image'&&block.image)return `<figure class="admin-preview-image ${block.size==='compact'?'compact':''} ${block.fit==='cover'?'cover':'contain'}"><img src="${attr(block.image)}" alt="${attr(block.alt||'')}">${block.caption?`<figcaption>${esc(block.caption)}</figcaption>`:''}</figure>`;
      if(block.type==='split'&&(block.image||block.body||block.heading)){
        ...
        return `<section class="admin-preview-flow ${block.imageSide==='left'?'image-left':'image-right'}"><figure class="admin-preview-flow-figure"><img class="${block.fit==='contain'?'contain':'cover'}" src="${attr(block.image)}" alt="${attr(block.alt||'')}">${block.caption?`<figcaption>${esc(block.caption)}</figcaption>`:''}</figure><div class="admin-preview-flow-copy">${block.heading?`<h2>${esc(block.heading)}</h2>`:''}${simpleRichPreview(block.body||'')}</div></section>`;
      }
      if(block.type==='gallery'){
        const images=(block.images||[]).filter(item=>item?.image);if(images.length<2)return'';
        return `<section class="admin-preview-gallery count-${Math.min(images.length,4)} ${block.layout||'auto'}">${images.map(item=>`<figure><img class="${block.fit==='contain'?'contain':'cover'}" src="${attr(item.image)}" alt="${attr(item.alt||'')}">${item.caption?`<figcaption>${esc(item.caption)}</figcaption>`:''}</figure>`).join('')}</section>`;
      }
      if(block.type==='cards'&&Array.isArray(block.items)){
        const iconMap={shield:'fa-shield-halved',syringe:'fa-syringe',leaf:'fa-leaf',home:'fa-house',heart:'fa-heart-pulse',paw:'fa-paw',bowl:'fa-bowl-food',star:'fa-star'};
        const items=block.items.filter(item=>item?.title||item?.body||item?.image).slice(0,3);if(!items.length)return'';
        const resolved=resolvePreviewDesign(data),titleKey=block.titleFont&&block.titleFont!=='default'?safeBlogFont(block.titleFont):resolved.cardTitleFont,bodyKey=block.bodyFont&&block.bodyFont!=='default'?safeBlogFont(block.bodyFont):resolved.cardBodyFont;
        return `<section class="admin-preview-card-list" style="--preview-card-title-font:${attr(BLOG_FONT_STACKS[titleKey])};--preview-card-body-font:${attr(BLOG_FONT_STACKS[bodyKey])}">${items.map(item=>`<article class="admin-preview-info-card card-size-${attr(item.size||'medium')} card-position-${attr(item.position||'auto')} card-text-${attr(item.textSize||'normal')} ${item.image?'has-media':''}"><div class="admin-preview-info-copy"><span><i class="fa-solid ${iconMap[item.icon]||iconMap.paw}"></i></span><div>${item.title?`<h3>${esc(item.title)}</h3>`:''}${item.body?simpleRichPreview(item.body):''}</div></div>${item.image?`<figure><img src="${attr(item.image)}" alt="${attr(item.alt||item.title||'')}"></figure>`:''}</article>`).join('')}</section>`;
      }
      if(block.type==='callout'&&block.body)return `<aside class="admin-preview-callout ${attr(block.tone||'info')}">${block.heading?`<h3>${esc(block.heading)}</h3>`:''}${simpleRichPreview(block.body)}</aside>`;
      if(block.type==='steps'&&Array.isArray(block.items)&&block.items.length)return `<section class="admin-preview-steps">${block.heading?`<h2>${esc(block.heading)}</h2>`:''}<ol>${block.items.map(item=>`<li>${esc(typeof item==='string'?item:item?.item||'')}</li>`).join('')}</ol></section>`;
      return'';
    }).filter(Boolean).join('');
    return sections||'<p>Henüz dolu bir bölüm yok.</p>';
  }
  function previewEditor(){
    const data=collectCurrentForm(),name=state.edit.name;let body='';
    if(name==='blog'){
      if(data.categoryMode!=='manual')data.category=inferBlogCategory(data).category||data.category;
      body=buildBlogPreviewHtml(data);
    }else body=`<h2>${esc(collectionTitle(name,data))}</h2><p style="line-height:1.7">${esc(collectionDescription(name,data))}</p>`;
    openModal({title:'Ön izleme',kicker:'YAYIN ÖNCESİ',body,footer:'<button class="button primary" id="previewClose">Kapat</button>'});$('#previewClose').addEventListener('click',closeModal);
  }
  ...
  async function permanentDelete(name,slug){const item=(await listCollection(name)).find(x=>x._slug===slug);if(!item||!item.trashed)return;if(!await confirmAction({title:'Kalıcı silme',message:`“${collectionTitle(name,item)}” kalıcı olarak silinsin mi? Bu işlem geri alınamaz.`,action:'permanent-delete-confirm',contentId:slug}))return;try{if(RUNTIME_COLLECTIONS.has(name)&&item._runtime){await runtimeRequest('/admin/permanent-delete',{method:'POST',body:JSON.stringify({type:name,slug:item._slug})});clearCollection(name);toast('İçerik kalıcı olarak silindi');await renderCollection(name);return;}await backupSave(item._path,item,item._sha,'Kalıcı silme öncesi son yedek');await deleteRaw(item._path,`Panel: kalıcı silindi — ${collectionTitle(name,item)}`,item._sha);clearCollection(name);toast('İçerik kalıcı olarak silindi');await renderArchive();}catch(error){toast('Silinemedi',error.message,'error');}}

  async function renderSettings(){
    let backupCount='—';try{const token=await state.user?.jwt?.();const r=await fetch('/.netlify/functions/admin-backups?summary=1',{headers:token?{Authorization:`Bearer ${token}`}:{},cache:'no-store'});if(r.ok)backupCount=(await r.json()).count;}catch{}
  ...
  document.addEventListener('click',event=>{
    ...
    confirmAction({title:'Kaydedilmemiş değişiklikler',message:'Bu sayfadan ayrılırsanız kaydedilmemiş değişiklikler kaybolur.',action:'leave-confirm',contentId:link.getAttribute('href')||''}).then(ok=>{if(ok){state.dirty=false;location.hash=link.getAttribute('href');}});
  });
