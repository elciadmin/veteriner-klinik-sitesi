(() => {
  ...
  async function renderCollection(name) {
    ...
    main.innerHTML = `
      ...
      <div class="toolbar">
        ...
        <select id="collectionStatus"><option value="active">Aktif içerikler</option><option value="all">Tümü</option><option value="published">Yayında</option><option value="scheduled">Planlandı</option><option value="draft">Yayında değil</option><option value="archived">Arşiv</option><option value="trash">Çöp kutusu</option></select>
    ...
    ['collectionSearch','collectionStatus','collectionCategory','collectionSort'].forEach(id => $(`#${id}`)?.addEventListener('input',render));
    render();
  }
  function collectionHelp(name) {
    return {
      ...
      announcements:'Ana sayfada süreli veya süresiz duyuru yayınlayın. Önceliği yüksek olan duyuru öne çıkar.',
  ...
  function renderCollectionList(name, originalItems) {
    ...
    const statusFilter = $('#collectionStatus')?.value || 'active';
    ...
    let items = originalItems.filter(item => {
      ...
      if (statusFilter === 'active' && ['archived','trash'].includes(status)) return false;
    ...
    if (!items.length) { list.innerHTML = emptyState('fa-magnifying-glass','Kayıt bulunamadı','Arama veya filtre seçiminizi değiştirin.'); return; }
  ...
  async function quickContentAction(name,slug,action) {
    ...
    if (action === 'publish') { data.published=true; data.archived=false; data.trashed=false; const field=COLLECTIONS[name].dateField; if(field && (!data[field] || dateValue(data[field])>new Date())) data[field]=nowIso(); }
    ...
    if (action === 'trash') { data.trashed=!item.trashed; if(data.trashed){data.published=false;data.archived=false;} }
    try {
      await writeJson(item._path,data,`Panel: ${collectionTitle(name,item)} — ${action}`,item._sha);
      clearCollection(name); toast('İçerik güncellendi', collectionTitle(name,item)); await renderCollection(name);
  ...
  function defaultItem(name) {
    if (name==='blog') return {published:true,featured:false,title:'',date:nowIso(),unpublishAt:'',category:BLOG_CATEGORIES[0],species:'Genel',tags:[],relatedService:'',summary:'',cover:'',content:'',editorialSections:[],advanced:{slug:'',seoTitle:'',seoDescription:'',youtubeId:'',author:'Elçi Veteriner Kliniği'}};
  ...
  function blogForm(data) {
    ...
    return `<section class="form-card"><h2>Yayın ayarları</h2><div class="form-grid">${switchField('published','Yayında',data.published!==false,'Kapattığınızda yazı panelde kalır, siteden kaldırılır.')}${switchField('featured','Ana sayfada öne çıkar',!!data.featured,'Ana sayfada en fazla üç yazı gösterilir.')}${inputField('date','Yayınlama zamanı',toInputDateTime(data.date),'datetime-local','Şimdi yayınlamak için mevcut zamanı bırakın.')}${inputField('unpublishAt','Otomatik yayından kalkma',toInputDateTime(data.unpublishAt),'datetime-local','Süresiz kalacaksa boş bırakın.')}</div></section>
      <section class="form-card"><h2>Yazının temel bilgileri</h2><div class="form-grid">${inputField('title','Başlık',data.title,'text','Okuyucunun aradığı konuyu açıkça anlatan başlık.',true,'required')}${selectField('category','Ana kategori',data.category,BLOG_CATEGORIES)}${selectField('species','Hayvan türü',data.species||'Genel',SPECIES)}${selectField('relatedService','İlgili hizmet',data.relatedService||'',[{label:'Seçilmedi',value:''},...RELATED_SERVICES])}${textField('summary','Kartta görünen kısa açıklama',data.summary,'1–3 kısa cümle. Okuyucunun yazıyı açma kararını kolaylaştırır.',true,3)}${imageField('cover','Kapak görseli',data.cover,'assets/img/uploads/blog','Yatay görsel önerilir. Büyük dosyalar otomatik küçültülür.')}</div></section>
      <section class="form-card"><h2>Etiketler</h2><p style="margin:-5px 0 12px;color:var(--muted);font-size:.7rem">Dağınıklığı önlemek için en fazla 8 hazır etiket seçin.</p><div class="multi-select">${BLOG_TAGS.map(tag=>`<label class="choice"><input type="checkbox" name="tags" value="${attr(tag)}" ${(data.tags||[]).includes(tag)?'checked':''}>${esc(tag)}</label>`).join('')}</div></section>
      <section class="form-card"><h2>Yazının içeriği</h2><div class="field full"><label for="f-content">Metin</label><div class="rich-toolbar"><button type="button" data-format="h2">Büyük başlık</button><button type="button" data-format="h3">Alt başlık</button><button type="button" data-format="bold">Kalın</button><button type="button" data-format="list">Madde</button><button type="button" data-format="number">Numara</button><button type="button" data-format="quote">Bilgi notu</button></div><textarea class="rich-text" id="f-content" name="content" placeholder="Hazır metni buraya yapıştırın…">${esc(data.content||'')}</textarea><small>Alt başlıklar ve kısa paragraflar okuma deneyimini iyileştirir.</small></div></section>
      <section class="form-card"><div class="item-card-head"><div><h2 style="margin:0">Estetik yazı bölümleri</h2><small style="color:var(--muted)">Görsel, bilgi kutusu veya adım listesi ekleyin.</small></div></div><div class="block-builder" id="editorialBuilder"><div class="block-toolbar"><button type="button" class="button" data-add-block="image"><i class="fa-regular fa-image"></i> Tam görsel</button><button type="button" class="button" data-add-block="split"><i class="fa-solid fa-table-columns"></i> Görsel + metin</button><button type="button" class="button" data-add-block="callout"><i class="fa-solid fa-circle-info"></i> Bilgi kutusu</button><button type="button" class="button" data-add-block="steps"><i class="fa-solid fa-list-ol"></i> Adımlar</button></div><div id="editorialBlocks">${(data.editorialSections||[]).map((block,index)=>renderEditorialBlock(block,index)).join('')}</div></div></section>
      <details class="advanced"><summary>SEO ve gelişmiş ayarlar</summary><div class="advanced-body"><div class="form-grid">${inputField('advanced.slug','Özel bağlantı adı',adv.slug||'','text','Boş bırakırsanız başlıktan otomatik oluşturulur.',true)}${inputField('advanced.seoTitle','Google başlığı',adv.seoTitle||'','text','Boşsa yazı başlığı kullanılır.',true)}${textField('advanced.seoDescription','Google açıklaması',adv.seoDescription||'','Arama sonucunda görünen kısa açıklama.',true,3)}${inputField('advanced.youtubeId','YouTube video kimliği',adv.youtubeId||'')}${inputField('advanced.author','Yazar',adv.author||'Elçi Veteriner Kliniği')}</div></div></details>`;
  }

  function renderEditorialBlock(block,index) {
    const type=block.type||'callout'; const typeLabel={image:'Tam genişlik görsel',split:'Görsel + metin',callout:'Bilgi kutusu',steps:'Numaralı adımlar'}[type];
    let fields='';
    if(type==='image') fields=`${blockImageField(index,block.image||'','Görsel')}${inputField(`block.${index}.alt`,'Görsel açıklaması',block.alt||'')}${inputField(`block.${index}.caption`,'Alt yazı',block.caption||'')}${selectField(`block.${index}.size`,'Genişlik',block.size||'wide',[{label:'Geniş',value:'wide'},{label:'Dar',value:'compact'}])}`;
    if(type==='split') fields=`${inputField(`block.${index}.heading`,'Bölüm başlığı',block.heading||'','text','',true)}${textField(`block.${index}.body`,'Metin',block.body||'','',true,5)}${blockImageField(index,block.image||'','Görsel')}${inputField(`block.${index}.alt`,'Görsel açıklaması',block.alt||'')}${selectField(`block.${index}.imageSide`,'Görsel konumu',block.imageSide||'right',[{label:'Sağda',value:'right'},{label:'Solda',value:'left'}])}`;
  ...
  function genericForm(name,data) {
    if(name==='announcements') return `<section class="form-card"><h2>Duyuru ayarları</h2><div class="form-grid">${switchField('published','Yayında',data.published!==false)}${switchField('showOnHome','Ana sayfada göster',data.showOnHome!==false)}${inputField('title','Başlık',data.title,'text','',true,'required')}${textField('message','Kısa duyuru metni',data.message,'',true,4)}${inputField('publishAt','Başlangıç zamanı',toInputDateTime(data.publishAt),'datetime-local')}${inputField('unpublishAt','Bitiş zamanı',toInputDateTime(data.unpublishAt),'datetime-local','Süresizse boş bırakın.')}${selectField('level','Görünüm',data.level||'info',[{label:'Bilgi',value:'info'},{label:'Önemli',value:'urgent'},{label:'Standart',value:'standard'}])}${inputField('priority','Öncelik',data.priority||10,'number','Büyük sayı öne çıkar.','', 'min="0" max="100"')}${inputField('linkLabel','Bağlantı yazısı',data.linkLabel||'')}${inputField('linkUrl','Bağlantı adresi',data.linkUrl||'','url')}${switchField('dismissible','Ziyaretçi kapatabilsin',data.dismissible!==false)}</div></section>`;
  ...
  function syncBlocksToState() {
    ...
    state.edit.data.editorialSections=$$('.editorial-block').map(block=>{
  ...
  function collectCurrentForm() {
    ...
    if(name==='announcements'){data.publishAt=fromInputDateTime(fd.get('publishAt'));data.unpublishAt=fromInputDateTime(fd.get('unpublishAt'));data.priority=Number(fd.get('priority')||0);}
  ...
  function addEditorialBlock(type) {
    const current=collectCurrentForm(); const defaults={image:{type:'image',image:'',alt:'',caption:'',size:'wide'},split:{type:'split',heading:'',body:'',image:'',alt:'',imageSide:'right'},callout:{type:'callout',tone:'info',heading:'',body:''},steps:{type:'steps',heading:'',items:[]}};
  ...
  function validateEditor(name,data,action) {
    if(!String(data.title||'').trim())return'Başlık alanını doldurun.';
    if(name==='blog'&&!String(data.summary||'').trim())return'Kısa açıklamayı yazın.';
    if(name==='blog'&&!String(data.content||'').trim())return'Yazının metnini ekleyin.';
    if(name==='faq'&&!String(data.answer||'').trim())return'Yanıtı yazın.';
    if(name==='reviews'&&(!data.author||!data.text))return'Yorum sahibi ve yorum metni gereklidir.';
    if(name==='instagram'&&!data.image)return'Instagram galerisi için görsel yükleyin.';
    const config=COLLECTIONS[name],date=config.dateField?dateValue(data[config.dateField]):null;
  ...
  function newPath(name,data) {
    ...
    const base=slugify(data.advanced?.slug||data.title||data.author);return`${config.folder}/${prefix}${base}.json`;
  }
  async function saveEditor(action) {
    ...
    if(action==='publish'){data.published=true;data.archived=false;data.trashed=false;if(config.dateField)data[config.dateField]=nowIso();}
    ...
    if(action==='trash'){data.published=false;data.archived=false;data.trashed=true;data.trashedAt=nowIso();}
    const validation=validateEditor(name,data,action);if(validation){toast('Eksik bilgi',validation,'warning');return;}
    ...
    try{
      ...
      state.dirty=false;toast('İçerik kaydedildi',actionLabel(action));location.hash=`#collection/${name}`;
  ...
  async function renderCalendar(){
    ...
    const events=[
      ...
      ...announcements.filter(x=>!x.trashed&&!x.archived).map(x=>({type:'announcement',date:x.publishAt,title:x.title,href:`#edit/announcements/${encodeURIComponent(x._slug)}`,status:contentStatus(x,COLLECTIONS.announcements).key})),
    ...
    main.innerHTML=`<header class="page-head"><div><span class="kicker">PAYLAŞIM PLANI</span><h1>İçerik takvimi</h1><p>Blog, duyuru ve Instagram galerisi kayıtlarını aynı takvimde görün. Boş günler için hazır fikirlerden yararlanın.</p></div><div class="page-actions"><a class="button primary" href="#edit/blog/new"><i class="fa-solid fa-plus"></i> Blog planla</a><a class="button" href="#edit/announcements/new">Duyuru planla</a></div></header>
      <section class="calendar-shell"><div class="panel calendar-panel"><div class="calendar-head"><button class="icon-button" id="calPrev"><i class="fa-solid fa-chevron-left"></i></button><h2>${esc(current.toLocaleDateString('tr-TR',{month:'long',year:'numeric'}))}</h2><div class="button-row"><button class="button" id="calToday">Bugün</button><button class="icon-button" id="calNext"><i class="fa-solid fa-chevron-right"></i></button></div></div>${calendarGrid(year,month,events)}</div><aside class="panel calendar-sidebar"><h2>Bu ay için içerik fikirleri</h2><p style="color:var(--muted);font-size:.7rem;line-height:1.6">Fikirler otomatik yayınlanmaz; yalnızca planlamayı hızlandırır.</p><div class="idea-list">${calendarIdeas(month).map(idea=>`<div class="idea-item"><strong>${esc(idea.title)}</strong><br><span style="color:var(--muted)">${esc(idea.text)}</span><div style="margin-top:8px"><a class="button" href="#edit/blog/new">Yazıya dönüştür</a></div></div>`).join('')}</div></aside></section>`;
    $('#calPrev').addEventListener('click',()=>{state.calendarDate=new Date(year,month-1,1);renderCalendar();});$('#calNext').addEventListener('click',()=>{state.calendarDate=new Date(year,month+1,1);renderCalendar();});$('#calToday').addEventListener('click',()=>{state.calendarDate=new Date();renderCalendar();});
  }
  function calendarGrid(year,month,events){const first=new Date(year,month,1),start=new Date(year,month,1-((first.getDay()+6)%7)),today=new Date();let cells='';for(let i=0;i<42;i++){const date=new Date(start);date.setDate(start.getDate()+i);const iso=[date.getFullYear(),String(date.getMonth()+1).padStart(2,'0'),String(date.getDate()).padStart(2,'0')].join('-');const dayEvents=events.filter(e=>{const d=dateValue(e.date);return d&&d.getFullYear()===date.getFullYear()&&d.getMonth()===date.getMonth()&&d.getDate()===date.getDate();});const isToday=date.toDateString()===today.toDateString();cells+=`<div class="calendar-day ${date.getMonth()!==month?'outside':''} ${isToday?'today':''}"><span class="calendar-date">${date.getDate()}</span><div class="calendar-events">${dayEvents.slice(0,4).map(e=>`<a class="calendar-event ${e.type}" href="${e.href}" title="${attr(e.title)}">${esc(e.title)}</a>`).join('')}${dayEvents.length>4?`<span class="calendar-event">+${dayEvents.length-4} içerik</span>`:''}</div></div>`;}return`<div class="calendar-grid">${['Pzt','Sal','Çar','Per','Cum','Cmt','Paz'].map(x=>`<div class="calendar-day-name">${x}</div>`).join('')}${cells}</div>`;}
  function calendarIdeas(month){const ideas={0:[['Yeni yıl sağlık kontrolü','Kedi ve köpeklerde yıllık check-up neden önemlidir?'],['Kışın su tüketimi','Evcil hayvanlarda su tüketimini artırma önerileri']],1:[['Ağız ve diş sağlığı','Diş taşı ve ağız kokusunda erken belirtiler'],['Kış yürüyüşleri','Soğuk havada pati ve deri bakımı']],2:[['Parazit dönemi','İlkbahar öncesi iç ve dış parazit planı'],['Mevsim geçişi','Tüy dökülmesi ne zaman normal değildir?']],3:[['Yavru bakımı','Yavru kedi ve köpekte ilk veteriner ziyareti'],['Mikroçip ve pasaport','Seyahat öncesi gerekli işlemler']],4:[['Sıcak hava hazırlığı','Sıcak çarpmasına karşı erken önlemler'],['Kene farkındalığı','Kene korumasında sık yapılan hatalar']],5:[['Yaz seyahati','Evcil hayvanla güvenli yolculuk rehberi'],['Su ve gölge','Yazın günlük bakım kontrol listesi']],6:[['Sıcak çarpması','Acil belirtiler ve ilk adımlar'],['Yaz iştahsızlığı','Ne zaman kliniğe başvurmalı?']],7:[['Aşı takvimi','Yavru ve yetişkinlerde hatırlatma dozları'],['Kulak ve deri','Yaz aylarında kulak ve deri sorunları']],8:[['Sonbahar check-up','Mevsim geçişinde genel sağlık kontrolü'],['Kilo kontrolü','Kış öncesi ideal kilo değerlendirmesi']],9:[['Hayvanları Koruma Günü','Sorumlu hayvan sahipliği ve koruyucu hekimlik'],['Diş bakımı','Evde ağız bakımına doğru başlangıç']],10:[['Diyabet farkındalığı','Kedi ve köpeklerde sık görülen belirtiler'],['Yaşlı hayvan bakımı','Senior dostlar için kontrol planı']],11:[['Yılbaşı güvenliği','Süsler, yiyecekler ve ev içi riskler'],['Kış bakımı','Soğuk havada yaşlı hayvanların korunması']]};return(ideas[month]||[]).map(([title,text])=>({title,text}));}
